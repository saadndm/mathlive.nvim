...
-- Clean screenshot line
Helpers.clean_line = function (line)
  return line:gsub("^%d+|", ""):gsub("%s+$", "")
end

-- Normalize screenshot text so MiniTest compares combining-mark cells consistently
Helpers.normalize_screenshot = function (screenshot)
  local normalized = vim.deepcopy(screenshot)

  for i, row in ipairs(normalized.text) do
    local joined = table.concat(row)
    normalized.text[i] = {}
    for j = 1, vim.fn.strchars(joined) do
      normalized.text[i][j] = vim.fn.strcharpart(joined, j - 1, 1)
    end
  end
